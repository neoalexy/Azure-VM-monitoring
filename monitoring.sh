...
while true; do
    ...
    mkdir -p /home/azureuser/log-backups
    BACKUPS="/home/azureuser/log-backups/log-backup-$(date '+%Y-%m-%d').tar.gz"
    tar --ignore-failed-read -czf "$BACKUPS" -C /var --exclude='log/journal/*' --exclude='log/grafana/*' --exclude='log/syslog' --exclude='log/sys-monitoring.log' log/
